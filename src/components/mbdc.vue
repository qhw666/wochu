<template>
  <div>
    ...
    <div class="swiper-item swiper-wrapper">
      <div v-for="item in list3" class="advice-group swiper-slide swiper-slide-active">
        <img :src="item.imgUrl" />
        <div class="group-name ellipsis">{{item.goodsName}}</div>
        <div class="m-price">
...
export default {
  ...
  created() {
    this.$axios
      ...
      .then(res => {
        this.list2 = res.data.data.acts[7].items;
        this.tit = res.data.data.acts[8].title;
      });
    this.$axios
      .get("http://api9.wochu.cn/api/app/acts?version=20.0.0&source=H")
      .then(res => {
        this.list3 = res.data.data.acts[8].items;
      });
  }
};
</script>
