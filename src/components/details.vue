...
<!--           <div class="swiper-pagination">111</div> -->
    ...
    <!-- 商品信息 价格 -->
    <div class="d-s1">
      <div class="d-s2" v-for="(item, index) in xiaourl" :key="index">
        <img :src="item.labelUrl" alt />
      </div>
      <h1>{{list.goodsName}}</h1>
      <h2>{{list.description}}</h2>
      <div class="d-s3">
        <div class="d-s4">￥{{list.price}}</div>
        <div class="d-s4-1">￥{{list.marketPrice}}</div>
      </div>
    </div>
    <div class="d-s5">
    ...
    <!-- 尾部  -->
    <div>
      <div v-html="list.descriptionDetail" class="d-s-wei"></div>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    getData() {
      this.$axios
        .get("http://api9.wochu.cn/api/goods/goods/detail?", {
          params: {
        ...
        .then(res => {
          ...
          this.xiaourl = res.data.data.goodsLabel;
          console.log(res.data.data.goodsLabel);
          /* console.log(res) */
        }),
        this.$axios
          .get(
            "http://api9.wochu.cn/client/v1/goods/getGoodsRelevantList?parameters=%7B%22goodsGuid%22:%227cb3b429-8ffd-4ebf-9a0c-48bfc8d169d9%22%7D"
          )
          .then(res => {
            /* console.log(res) */
        ...
        this.$axios
          .get("http://api9.wochu.cn/client/v1/goods/imgLoopList?", {
            params: {
